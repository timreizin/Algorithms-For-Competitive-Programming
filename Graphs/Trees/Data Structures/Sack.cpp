...
private:
    ...
    int calculate(int v, int p, int &t)
    {
        ...
        tin[v] = t++;
        sizes[v] = 1;
        for (int u : adj[v]) if (u != p) sizes[v] += calculate(u, v, t);
        tout[v] = t - 1;
        return sizes[v];
    }
    ...
    void dfs(int v, int p, bool isHeavy = false)
    {
        ...
        for (int u : adj[v]) if (u != p && u != heavy) dfs(u, v);
        ...
        if (!isHeavy)
        {
            for (int i = tin[v]; i <= tout[v]; ++i) answer.update(values[tTov[i]], -1, 1);
            answer.clear();
        }
    }
...
public:
    ...
    Sack(vector<vector<int>> adj, vector<int> values) : adj(adj), values(values)
    {
